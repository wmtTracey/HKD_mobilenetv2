...
def train(config, train_loader, model, criterion, optimizer, writer_dict, epoch=0, device="cuda:0"):
    ...
    for i, (input, target, target_weight, meta) in enumerate(train_loader):
        ...
        if i % config['PRINT_FREQ'] == 0:
            ...
            msg = 'Epoch: [{0}][{1}/{2}]\t' \
                  'ValTime {batch_time.val:.3f}s (avg{batch_time.avg:.3f}s)\t' \
                  'Speed {speed:.1f} images/s\t' \
                  'Data {data_time.val:.3f}s ({data_time.avg:.3f}s)\t' \
                  'ValLoss {loss.val:.5f} (avg{loss.avg:.5f})\t' \
                  'ValAccuracy {acc.val:.3f} (avg{acc.avg:.3f})\t' \
                  'lr:{lr:.7f}'.format(
                epoch, i, num_samples, batch_time=batch_time,
...
def validate(config, val_loader, model, criterion, writer_dict, epoch=0, device="cuda:0"):
    batch_time = AverageMeter()
    losses = AverageMeter()
    acc = AverageMeter()
    output_dir = config['WORK_DIR']
    ...
    with torch.no_grad():
        ...
        for i, (input, target, target_weight, meta) in enumerate(val_loader):
            ...
            image_id = np.asarray(meta['image_id']).tolist()
        ...
        name_values, ap = val_dataset.evaluate(config, all_preds, output_dir, all_boxes, image_path,
                                               filenames, imgnums, image_idx=image_idx)

        if isinstance(name_values, list):
            for name_value in name_values:
                _print_name_value(name_value, config['MODEL']['NAME'])
        else:
