...
from models.tools.transforms import transform_preds, transform_preds_offset
...
def get_final_preds_offset(config, batch_heatmaps, center, scale):
    heatmap_height = batch_heatmaps.shape[2]
    heatmap_width = batch_heatmaps.shape[3]
    ...
    # Transform back
    for i in range(coords.shape[0]):
        preds[i] = transform_preds_offset(coords[i], center[i], scale[i], [heatmap_width, heatmap_height])
        # preds[i] = transform_preds(coords[i], center[i], scale[i], [heatmap_width, heatmap_height])
